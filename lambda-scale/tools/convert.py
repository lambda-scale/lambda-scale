...
def convert_model(
    ...
    loaded = [torch.load(os.path.join(input_base_path, file)) for file in checkpoint_list]
    param_count = 0
    ...
    dir_path = f'{root_path}/gpu-fast-scaling/test_bed_local/serve/server/model_storage/{model_name}'
...
if __name__ == '__main__':
    ...
    for id,distribution in enumerate(device_distribution):
        for block_id in distribution:
            device_map[block_id] = get_gpu_id(node_id=1,
                                            worker_id=0,
                                            gpu_num=gpu_num,
                                            id=id)

    convert_model(root_path=root_path,
        input_base_path = f'{root_path}/gpu-fast-scaling/test_bed_local/serve/model_info/models/llama/llama-2-7b/',
        num_shards = 1,
