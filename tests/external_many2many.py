...
# launch_client(0, 0)
# for i in range(4):
#     for j in range(1, server_num):
#         load_model(0, j)
#         time.sleep(1)
# exit(0)

"""
Test on-demand load pipeline
"""
def ondemand_execute(func, server_id):
    if func in func_server_map:
        if server_id in func_server_map[func]:
            return
        source_server_id = func_server_map[func][0]
        ...
        ack = send_signal(req, server_id)
        end_t = time.time()
        print(f'Ondemand execute {func} on server {server_id} from source {source_server_id} signal ack {ack.ack} time {end_t - start_t}')

        start_t = time.time()
        x = requests.get('http://localhost:' + str(9000 + func), headers={"cur_server" : str(server_id)})
        end_t = time.time()
        print(f'Execute {func} on server {server_id} time {end_t - start_t} resp: {x.text}')
        time.sleep(1)
