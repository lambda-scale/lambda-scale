...
export TMPDIR="${INSTALL_PREFIX}/tmp/mutils-containers"
mkdir -p "${TMPDIR}"
WORKPATH="${TMPDIR}"
...
# Clone the mutils-containers repository
git clone https://github.com/mpmilano/mutils-containers.git "${TARGET_DIR}"
cd "${TARGET_DIR}"
...
if make install; then
    rm -rf "${WORKPATH}"
    echo "mutils-containers has been successfully installed to ${INSTALL_PREFIX}"
else
