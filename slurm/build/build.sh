...
cmake_defs="-DCMAKE_BUILD_TYPE=${build_type} -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DCMAKE_INSTALL_PREFIX=${install_prefix} -DCMAKE_PREFIX_PATH=/scratch/infattllm/fgscaling/rui/slurm_nodes/workspace/RDMC-GDR/third_party;/usr/lib64;/cm/shared/apps/cuda12.2/toolkit/12.2.2"
...
echo "recursively build submodules....."
git submodule init
...
#make -j $(awk "BEGIN {print int($(nproc) * 0.7)}")>err.log

echo "=========moving rdmc_shn share object to ./pyrdmc/========="
mv ./rdmc_shn*.so ../pyrdmc
echo "======moving successfully========"
cd ..
