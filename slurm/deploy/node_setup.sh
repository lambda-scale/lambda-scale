...
configure_gfs() {
    ...
    # 30000000000 for 7b/13b, 150000000000 for 70b
    # host_memory_size: pin memory (cpu)
    sed -i "s#^host_memory_size = .*#host_memory_size = 30000000000#" evaluation.cfg

    #is_nccl_impl:
    sed -i "s#^is_nccl_impl = .*#is_nccl_impl = False#" evaluation.cfg
    sed -i "s#^is_nccl = .*#is_nccl = False#" evaluation.cfg
    sed -i "s#^is_faasnet = .*#is_faasnet = True#" evaluation.cfg
    sed -i "s#^is_sllm = .*#is_sllm = False#" evaluation.cfg

    sed -i "s#^is_ideal = .*#is_ideal = False#" evaluation.cfg

    sed -i "s#^is_sys_ablation_tensor_pack = .*#is_sys_ablation_tensor_pack = False#" evaluation.cfg
    sed -i "s#^is_sys_ablation_pre_alloc = .*#is_sys_ablation_pre_alloc = False#" evaluation.cfg
    sed -i "s#^is_sys_ablation_memory_gdr = .*#is_sys_ablation_memory_gdr = False#" evaluation.cfg

    #options: [llama-7b, 0], [llama-13b, 1], [llama-13b, 2]
    sed -i "s#^model_name = .*#model_name = llama-2-13b#" evaluation.cfg
    # what is model_id for 7b, 13b and 70b respectively
    sed -i "s#^model_id = .*#model_id = 0#" evaluation.cfg

    # if_init_data: warm-up switch [enable/disable]
    sed -i "s#^if_init_data = .*#if_init_data = True#" evaluation.cfg
    #options: 0 | 1 | 2 ...
    sed -i "s#^init_node_num = .*#init_node_num = 1#" evaluation.cfg

    #options: default_ssd -> True
    sed -i "s#^default_ssd = .*#default_ssd = True#" evaluation.cfg
    sed -i "s#^memory_num = .*#memory_num = 0#" evaluation.cfg

    sed -i "s#^is_memory_keep_alive = .*#is_memory_keep_alive = False#" evaluation.cfg
    sed -i "s#^memory_keep_alive_time = .*#memory_keep_alive_time = 0#" evaluation.cfg

    #options: default_memory has to be false
    #assume all models are in cpu memory
    #Rui: no need to change below two options
    sed -i "s#^default_memory = .*#default_memory = False#" evaluation.cfg
    sed -i "s#^default_remote_storage = .*#default_remote_storage = False#" evaluation.cfg

    # control to use trace or not
    # Rui: has to be true for none trace experiments
    sed -i "s#^fixed_evaluation = .*#fixed_evaluation = True#" evaluation.cfg
    #options: 0 | 1 | 2 ...
    sed -i "s#^scale_node_num = .*#scale_node_num = 3#" evaluation.cfg

    sed -i "s#^is_trace = .*#is_trace = False#" evaluation.cfg
    sed -i "s#^rps = .*#rps = 0#" evaluation.cfg

    sed -i "s#^is_disable_cache = .*#is_disable_cache = False#" evaluation.cfg
    sed -i "s#^is_disable_execute = .*#is_disable_execute = True#" evaluation.cfg
    sed -i "s#^is_disable_reorder = .*#is_disable_reorder = False#" evaluation.cfg
    sed -i "s#^is_half_reorder = .*#is_half_reorder = False#" evaluation.cfg
...
start_workers() {
    ...
    nohup bash $WORKER_FILE &> "../log/worker_${NODE_ID}.log" &
